...
- **定性评审模式**（defects/ 无文件）：requirements / test_cases / tech_designs 任一目录有文件即触发对应评审——**需求评审、测试用例评审、技术方案评审**（可组合），各出独立评审报告（md 交付，档案风 HTML 可选）
...
6. **生成 HTML**（仅复盘模式必需）：`python scripts/cli.py html --root ...` 生成汇总仪表板；评审模式以 md 为最终产物，如需档案风 HTML 可选 `--doc <md路径>`。
...
- `{系统名}_{迭代号}_复盘报告_产品_{ts}.md/.json` —— 8 个固定章节（需求评审、变更连锁、加急质量、依赖缺失、需求溯源、交互体验、遗留风险等）
...
- `{系统名}_{迭代号}_复盘报告_汇总_{ts}.html` —— **单文件自包含 HTML 汇总报告**（仪表板模式：总览读数窗 + 帕累托 + 能量谱 + 三类产物全部章节），零外部依赖，可离线打开、下载、迁移分享，支持打印分页
- `{系统名}_{迭代号}_需求评审报告_{ts}.md` —— 需求评审产物（四维评估 + 结构拆解 + 交互体验 + P0/P1/P2 澄清清单；HTML 可选）
- `{系统名}_{迭代号}_用例评审报告_{ts}.md` —— 用例评审产物（结构完整性 + 覆盖设计 + 文案可判定性 + 澄清清单；HTML 可选）
- `{系统名}_{迭代号}_技术评审报告_{ts}.md` —— 技术方案评审产物（方案完整性 + 风险识别 + 可测试性评估 + 澄清清单；HTML 可选）
