...
└── scripts/
    ...
    ├── docx_extract.py          # docx 需求文档文本提取（标准库，含删除线检测）
...
- **纯需求定性模式**：仅有需求文档且用户明确要求时，跳过量化管线，AI 直读文档输出独立的需求评审报告。
...
| requirements/ | 需求文档（**docx 支持自动提取**：`python scripts/docx_extract.py "<路径>"`，删除线段落带「[删除线]」前缀用于识别废弃范围）；如含 `需求变更记录.csv`（列：需求ID、变更内容）可统计变更连锁缺陷 | 可选 |
...
- `需求评审报告_{版本}_{ts}.md` —— 纯需求定性模式产物（四维评估 + 结构拆解 + 交互体验 + 测试视角澄清清单）
